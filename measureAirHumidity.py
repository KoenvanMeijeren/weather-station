...
try:
    while True:
        ...
        if verbose:
            print("%s: %s %s" % (sensor_data[1], humidity, sensor_data[2]))
        
        # Save the readed data from the sensor in the database.
        try:
            cursor.execute("INSERT INTO measurement (value, sensor_id) VALUES (%s, %s);", [humidity, sensor_data[0]])
        except mariadb.Error as error:
            print("Error: {}".format(error))
            sys.exit(2)
